                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="full-width-show-view">
                            ...
                            {{-- News Articles --}}
                            <ul class="list-group list-group-flush mb-4 shadow-sm" style="max-width: 860px">
                                @forelse($newsArticles as $newsArticle)
                                    <li class="list-group-item py-4">
                                        ...
                                        @endif
                                        @if($newsArticle->people->count() > 0)
                                            <p class="text-info my-1">
                                                <i class="fad fa-user"></i>
                                                @foreach ($newsArticle->people as $item)
                                                    {{ $item->name }}@if (!$loop->last) / @endif
                                                @endforeach
                                            </p>
                                        @endif
                                        @if($newsArticle->companies->count() > 0)
                                            <p class="text-info my-1">
                                                <i class="fad fa-building"></i>
                                                @foreach ($newsArticle->companies as $item)
                                                    {{ $item->name }}@if (!$loop->last) / @endif
                                                @endforeach
                                            </p>
                                        @endif
