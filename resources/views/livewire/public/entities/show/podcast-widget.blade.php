<div class="mb-5 d-lg-flex">
    <div class="d-flex flex-column flex-md-row border-bottom pb-5">
        <div class="order-2 order-md-1 flex-shrink-0 mt-3 mt-md-0 me-3 me-lg-4">
            <img src="{{ $record->source->entity_image_url ?? asset('images/image-placeholder-podcast.png') }}" alt="{{ $record->source->name }}" class="d-none d-md-block podcast-image mb-3">
            <div class="d-md-flex flex-column justify-content-lg-center align-items-lg-center">
                <a href="" class="btn btn-primary rounded-0 me-2 me-md-0 mb-md-2 text-nowrap">
            ...
            @if($record->companies->count() > 0 OR $record->people->count() > 0)
                <div class="d-flex flex-wrap align-items-center justify-content-start">
                    @foreach($record->companies as $company)
                        <a href="{{ $company->show_url }}" class="d-block mt-3 me-3" title="{{ $company->name }}">
                    ...
                    @foreach($record->people as $person)
                        <a href="{{ $person->show_url }}" class="d-block mt-3 me-3">
