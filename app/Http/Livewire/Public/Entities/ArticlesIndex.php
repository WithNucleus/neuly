...
class ArticlesIndex extends Component
{
    ...
    public function updatedCompanySearch() {
        $this->returnCompanySearch('articles');
    }

    public function updatedPersonSearch() {
        $this->returnPersonSearch('articles');
    }

    public function updatedSourceSearch() {
        $this->returnSourceSearch('articles');
    }

    public function getRowsQueryProperty()
    {
        $query = MediaItem::articles()->with(['companies', 'people', 'focus', 'source'])
    ...
    {
        return view('livewire.public.entities.articles-index', [
            'records' => $this->rows,
            'focusOptions' => Focus::whereHas('articles')->withCount('articles')->orderByDesc('articles_count')->get()->toArray()
        ]);
    }
}
