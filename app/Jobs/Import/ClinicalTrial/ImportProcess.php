...
class ImportProcess implements ShouldQueue
{
    ...
    public function __construct(ImportedEntity $importedEntity)
    {
    ...
    public function handle()
    {
        if ($this->importedEntity->importable) {
            $clinicalTrial = $this->importedEntity->importable;
            $clinicalTrial->update($this->getDataAttributes());
        } else {
            $match = Clinicaltrial::where('nct_number', $this->importedEntity->name)->first();

            if ($match) {
                $clinicalTrial = $match;
                $this->importedEntity->importable_id = $clinicalTrial->id;
        ...
        $this->matchConditionRelationships($clinicalTrial);
        $this->matchInterventionRelationships($clinicalTrial);
        $this->matchLeadSponsor($clinicalTrial);
        $this->matchResponsibleParty($clinicalTrial);
        $this->matchCollaborators($clinicalTrial);
        $this->maskingInfo($clinicalTrial);
        $this->getArmsGroups($clinicalTrial);
        $this->getAgeGroups($clinicalTrial);
        $this->matchPhaseRelationships($clinicalTrial);

        $this->importedEntity->refresh();

        if ($this->importedEntity->errors) {
            $this->importedEntity->status = ImportedEntity::STATUS_WITH_ERRORS;
        } else {
            $this->importedEntity->status = ImportedEntity::STATUS_COMPLETE;
        }
    ...
    private function getDataAttributes(): array
    {
        ...
        $attributes['official_title'] = $this->getField('official_title', $this->importedEntity->data['protocolSection']['identificationModule']);
    ...
    private function getArmsGroups($clinicalTrial) {
        ...
        $clinicalTrial->arm_groups = $prettyGroups;
        $clinicalTrial->save();
    }

    private function getAgeGroups($clinicalTrial) {
        if (array_key_exists('stdAges', $this->importedEntity->data['protocolSection']['eligibilityModule'])) {
            ...
            $clinicalTrial->age_groups = $prettyAgeGroups;
            $clinicalTrial->save();
        }
    }
    ...
    private function matchConditionRelationships($clinicalTrial) {
        ...
        $clinicalTrial->conditions()->sync($recordIds);
    }
    ...
    private function matchInterventionRelationships($clinicalTrial) {
        ...
        foreach ($items as $item) {
            ...
            if ($focus) {
                $clinicalTrial->focus()->syncWithoutDetaching($focus->id);
            }
        ...
        $clinicalTrial->interventions()->sync($recordsWithPivots);
    }

    private function matchPhaseRelationships($clinicalTrial) {
        ...
        $clinicalTrial->phases()->sync($recordIds);
    }

    private function matchResponsibleParty($clinicalTrial) {

        if ($clinicalTrial->responsible_party_id AND $clinicalTrial->responsible_party_type) {
            return;
        }

        if (!array_key_exists('responsibleParty', $this->importedEntity->data['protocolSection']['sponsorCollaboratorsModule'])) {
            $clinicalTrial->responsible_party_notes = "No responsible party listed";
            $clinicalTrial->save();
            return;
        }
        ...
        if ($responsiblePartyType === 'SPONSOR') {
            $clinicalTrial->lead_sponsor_notes = 'Lead sponsor is responsible party';
        } else {
            ...
            if ($person) {
                ...
                $clinicalTrial->responsible_party_id = $person->id;
                $clinicalTrial->responsible_party_type = Person::class;
            } else {
    ...
    private function matchCollaborators($clinicalTrial) {
        if (array_key_exists('collaborators', $this->importedEntity->data['protocolSection']['sponsorCollaboratorsModule'])) {
            ...
            $clinicalTrial->collaborators()->syncWithPivotValues($collaboratorIds, ['type' => Clinicaltrial::COLLABORATOR]);
        }
    }

    private function matchLeadSponsor($clinicalTrial) {
        ...
        if ($companyMatch) {
            $clinicalTrial->lead_sponsor_id = $companyMatch->id;
            $clinicalTrial->lead_sponsor_type = Company::class;
        ...
        if ($personMatch) {
            $clinicalTrial->lead_sponsor_id = $personMatch->id;
            $clinicalTrial->lead_sponsor_type = Person::class;
    ...
    private function maskingInfo($clinicalTrial) {
        if (!array_key_exists('maskingInfo', $this->importedEntity->data['protocolSection']['designModule']['designInfo'])) {
            return;
        }

        $clinicalTrial->masking = $this->getField('masking', $this->importedEntity->data['protocolSection']['designModule']['designInfo']['maskingInfo']);
        $clinicalTrial->masking_description = $this->getField('maskingDescription', $this->importedEntity->data['protocolSection']['designModule']['designInfo']['maskingInfo']);
        ...
        if (array_key_exists('whoMasked', $this->importedEntity->data['protocolSection']['designModule']['designInfo']['maskingInfo'])) {
            ...
            $clinicalTrial->who_masked = $prettyMaskedArray;
        }
