...
@section('content')
    <!-- Default box -->
    <button href="{{ url(config('backpack.base.route_prefix', 'admin').'/backup/create') }}"
            class="btn btn-primary ladda-button mb-2 js-create-new-backup-button" data-style="zoom-in"><span class="ladda-label"><i
                class="la la-plus"></i> {{ trans('backpack::backup.create_a_new_backup') }}</span></button>
    <button href="{{ route('backup.database.store') }}"
            class="btn btn-primary ladda-button mb-2 js-create-new-backup-button" data-style="zoom-in"><span class="ladda-label"><i
                class="la la-plus"></i> Create new DB backup</span></button>
    <div class="card">
                            ...
                            @if ($b['download'])
                                <a class="btn btn-sm btn-link"
                                   href="{{ url(config('backpack.base.route_prefix', 'admin').'/backup/download/') }}?disk={{ $b['disk'] }}&path={{ urlencode($b['file_path']) }}&file_name={{ urlencode($b['file_name']) }}"><i
                                        class="la la-cloud-download"></i> {{ trans('backpack::backup.download') }}</a>
                            @endif
...
@section('after_scripts')
    ...
    <script>
        jQuery(document).ready(function ($) {
            ...
            $(".js-create-new-backup-button").click(function (e) {
                e.preventDefault();
